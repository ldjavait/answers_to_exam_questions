...
- Создание объекта абстрактного класса может привести к ошибкам во время выполнения, так как попытка создания такого
  объекта приведет к исключению времени выполнения (runtime exception).
...
4) Абстрактные классы используются тогда, когда есть отношение IS-A, т.е. класс наследник расширяет базовый класс
...
однотипных иерархий. В этом случает наследование от абстрактного класса реализующего поведение объекта по умолчанию
...
Статический вложенный класс это вложенный класс в объявлении которого есть ключевое слова `static`. Как и другие
...
- до java 6 локальные классы не могли содержать в себе статические объявления, допустимы только переменные константы. В
...
использования. Создание экземпляра анонимного класса происходит одновременно с кго объявлением(между скобками и точкой с
...
`Error`  ошибки представляют собой серьезные проблемы которые, согласно спецификации java, не следует обрабатывать,
поскольку они связанны с проблемами уровня JVM. Например: закончилась доступная память, доступная JVM(StackOverflow
error).
...
так он унаследован от `java.lang.Record`. Но record может реализовывать интерфейсы (интерфейсы мы будем проходить в
разделе Полиморфизм, пока что просто можно запомнить, что record работает с интерфейсами). Record удобно применять,
когда
...
заканчивается для записи в существующем массиве символов создается новый массив в 2 раза большей длинны и в него
копируется все содержимое старого массива. Теперь мы не создаем новый объект при каждом проходу цикла, а просто
...
- \r - Возврат курсора в начало строки. Возвращает курсор в начало строки, "обнуляя" её. Если в строке были данные, они
  пропадут.
- \n - Перевод курсора на новую строку.
- \f - Прогон страницы. Переводит документ на новую страницу. Неиспользуемый спецсимвол.
- \b - Возврат курсора на одну позицию назад. Работает как BackSpace - стирает один символ позади текущего положения
  курсора.
...
printf() выполняет форматирование и выводит данные в консоль без сохранения. Для более глубокого понимания процесса в
этом уроке вывод делается через цепочку методов (println + format). Все эти методы можно переписать с помощью printf() и
получить такой же результат за исключением отсутствия у метода printf() перевода курсора на новую строку.
...
аргументов, а также сами значения, которые необходимо вставить. Форматирование применяется в шаблонных текстах или
строках, где требуется подстановка в какие-то места шаблона различных значений, принимаемых в виде параметров.
...
%s, это некий шаблон. Спецификатор %s здесь означает тип аргумента, который будет передан на форматирование (
подстановку). Он означает любой тип параметра, который будет приведён к строке. Это самый универсальный вариант
подстановки значения в параметр. Вторым аргументом в данной строке передано значение для подстановки - строка name,
которую принимает метод getGreeting(). В результате метод format() создает строку по указанному шаблону с подставленным
значением.

- С помощью спецификатора d% мы передаём на форматирование целое число.
- %b. Любой переданный аргумент будет
  превращён в true.
- %c. Задаёт любой символ из таблицы Юникод. %c. Задаёт любой символ из таблицы Юникод.
